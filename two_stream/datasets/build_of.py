...
def run_optical_flow(vid_item):
    ...
    # se un video l'ho già fatto non lo rifaccio
    if not os.path.isdir(out_full_path):
        ...
        cmd = os.path.join(df_path + 'extract_gpu')+' -f={} -x={} -y={} -i={} -b=20 -t=1 -d={} -s=1 -o={} -w={} -h={}'.format(
            quote(vid_path), quote(flow_x_path), quote(flow_y_path), quote(image_path), dev_id, out_format, new_size[0], new_size[1])

        os.system(cmd)
        print('{} done -> [{}/{}]'.format(vid_name, len(os.listdir(out_path)), len(vid_list)))
        sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--df_path", type=str, default='./two_stream/',
