...
def VideoTemporalPrediction(
    ...
    if num_frames == 0:
        # print(vid_name)
        imglist = glob.glob(os.path.join(vid_name, 'flow_x_*.jpg'))
    ...
    # nel caso in cui ci siamo meno di 25*10 flow x o y
    if num_samples*10 > duration:
        if optical_flow_frames <= duration:
            step = 1
            num_samples = int(math.floor(duration/optical_flow_frames))
        else:
            step = 1
            num_samples = 1
            n_mancanti = optical_flow_frames - duration
            img_to_copy_x = cv2.imread(os.path.join(vid_name, f'flow_x_{duration:05d}.jpg'))
            img_to_copy_y = cv2.imread(os.path.join(vid_name, f'flow_y_{duration:05d}.jpg'))
            for i in range(duration+1, duration + n_mancanti+1):
                cv2.imwrite(os.path.join(vid_name, f'flow_x_{i:05d}.jpg'), img_to_copy_x)
                cv2.imwrite(os.path.join(vid_name, f'flow_y_{i:05d}.jpg'), img_to_copy_y)
