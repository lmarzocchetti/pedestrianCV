...
model_names = sorted(name for name in models_res.__dict__
    if name.islower() and not name.startswith("__")
    and callable(models_res.__dict__[name]))
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.save_freq == 0:
            checkpoint_name = "%03d_%s" % (epoch + 1, "checkpoint_frame.pth.tar")
            save_checkpoint({
                'epoch': epoch + 1,
                'arch': args.arch,
...
def build_model():
    # aggiunto per continuare il training dalla 50-esima epoca
    if args.start_epoch != 0:
        params = torch.load('./checkpoints/050_checkpoint_flow.pth.tar')
        model = models_res.flow_resnet152(pretrained=False, num_classes=101)
        model.load_state_dict(params['state_dict'])
    else:
        model = models_res.__dict__[args.arch](pretrained=True, num_classes=101)
    model.cuda()
    return model
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # measure accuracy and record loss (cambiato da += prec1[0] a prec1 e da loss.data[0] a loss.data)
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            # volatile=True è deprecato -> si usa with torch.no_grad():
            ...
            # measure accuracy and record loss (cambiato loss.data[0] prec1[0] prec3[0])
            prec1, prec3 = accuracy(output.data, target, topk=(1, 3))
...
def save_checkpoint(state, is_best, filename, resume_path):
    ...
    best_path = os.path.join(resume_path, 'model_best_frame.pth.tar')
