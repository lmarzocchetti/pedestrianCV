...
from cv2 import imwrite, cvtColor, COLOR_RGB2BGR, imread
...
def detect_directory(model, video_id, weights_path, img_path, classes, output_path, n_frames, tracker,
    ...
    video_dict = _draw_and_save_detected_video(video_id,
...
def _draw_and_save_detected_video(video_id, img_detections, imgs, tracker, img_size, output, classes, n_frames):
    ...
    for (image_path, detections) in tqdm(zip(imgs, img_detections), desc="Creation of dictionary..."):
        if i == n_frames:
            break
        #print(f"Image {image_path}:")
        video_dict = frame_dict(
            video_dict, image_path, detections, tracker, img_size)
        #detected_frames.extend([detected_frame])
        frames.extend((i, image_path))
        i += 1
    
    # Create folder where to save videos for each person
    path_to_person_videos = f"deep_sort/person_videos/{video_id}"
    makedirs(path_to_person_videos, exist_ok=True)
    # Iterate through keys of the dictionary (the people)
    offset = 50
    for person in video_dict:
        print(f"Composing video for person {person}...")
        person_dict = video_dict[person]
        person_video = []
        for frame_cuts in person_dict:
            frame = imread(frame_cuts)
            frame = cvtColor(frame, COLOR_RGB2BGR)    
            x1, y1, x2, y2 = person_dict[frame_cuts]
            frame = frame[y1-offset:y2+offset, x1-offset:x2+offset]
            person_video.extend([ImageClip(frame).set_duration(35 / 1050)])
        concat_clip = concatenate_videoclips(person_video, method="compose")
        concat_clip.write_videofile(f"{path_to_person_videos}/{person}.mp4", fps=30)
        print(f"Saved to {path_to_person_videos}/{person}.mp4")
                
    # Take video dict and make mini-videos for all the people
...
def frame_dict(video_dict, image_path, detections, tracker, img_size):
    ...
    for x1, y1, x2, y2, conf, cls_pred in detections:
        ...
        if (len(outputs) > 0):
            ...
            video_dict = detection_dict(bbox_xyxy, video_dict, image_path, identities)
    """
...
# function to convert video to dict
def detection_dict(bbox, video_dict, frame_id, identities=None, offset=(0,0)):
    for i, box in enumerate(bbox):
        ...
        y2 += offset[1]
        # box text and bar
        _id = identities[i] if identities is not None else 0
        if not video_dict.get(_id):
            video_dict[_id] = dict()
        video_dict[_id][frame_id] = (x1, y1, x2, y2)
...
def run(args):
    ...
    # Initialize deepsort
    deepsort = get_deepsort("deep_sort/deep_sort/deep/checkpoint/ckpt.t7")
        
    video_dict = video_inference(
        args.model,
        args.weights,
        args.videos,
        classes,
    ...
    # video_dict_transformed = {}
    
    # for id, inner in video_dict.items():
    #     for file, bbox in inner.items():
    #         num = int(file.split('/')[2].removesuffix('.jpg'))
    #         if id in video_dict_transformed.keys():
    #             video_dict_transformed[id].append((num, bbox))
    #         else:
    #             video_dict_transformed[id] = []
    #             video_dict_transformed[id].append((num, bbox))
        
    # print(video_dict_transformed)
